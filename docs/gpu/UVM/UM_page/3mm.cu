...
int main(int argc, char** argv)
{
	...
	if(strcmp(argv[1], "ZC") == 0){
		...
		if(size1 != 0){
			...
			CUDA_CHECK(cudaMemAdvise(F, size1, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
			CUDA_CHECK(cudaMemAdvise(F, size1, cudaMemAdviseSetAccessedBy, 0));
			CUDA_CHECK(cudaMemAdvise(G, size1, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
			CUDA_CHECK(cudaMemAdvise(G, size1, cudaMemAdviseSetAccessedBy, 0));
		}
		...
		if(size1 != 0){
			...
			CUDA_CHECK(cudaMemAdvise(F, size1, cudaMemAdviseSetPreferredLocation, 0));
			CUDA_CHECK(cudaMemPrefetchAsync(F, size1, 0, 0));
			CUDA_CHECK(cudaMemAdvise(G, size1, cudaMemAdviseSetPreferredLocation, 0));
			CUDA_CHECK(cudaMemPrefetchAsync(G, size1, 0, 0));
		}
	...
	if(strcmp(argv[3], "ZC") == 0){
		...
		if(size2 != 0){
			...
			CUDA_CHECK(cudaMemAdvise((F+count1), size2, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
			CUDA_CHECK(cudaMemAdvise((F+count1), size2, cudaMemAdviseSetAccessedBy, 0));
			CUDA_CHECK(cudaMemAdvise((G+count1), size2, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
			CUDA_CHECK(cudaMemAdvise((G+count1), size2, cudaMemAdviseSetAccessedBy, 0));
		}
		...
		if(size2 != 0){
			...
			CUDA_CHECK(cudaMemAdvise((F+count1), size2, cudaMemAdviseSetPreferredLocation, 0));
			CUDA_CHECK(cudaMemPrefetchAsync((F+count1), size2, 0, 0));
			CUDA_CHECK(cudaMemAdvise((G+count1), size2, cudaMemAdviseSetPreferredLocation, 0));
			CUDA_CHECK(cudaMemPrefetchAsync((G+count1), size2, 0, 0));
		}
	}else if(strcmp(argv[1], "DP") == 0){

	}

	mm3_kernel1<<<grid1,block>>>(A, B, E);
	CUDA_CHECK(cudaDeviceSynchronize());

	mm3_kernel2<<<grid2,block>>>(C, D, F);
	CUDA_CHECK(cudaDeviceSynchronize());

	mm3_kernel3<<<grid3,block>>>(E, F, G);
	CUDA_CHECK(cudaDeviceSynchronize());
