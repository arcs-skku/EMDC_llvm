...
//    memset(tmp_output_hidden_cuda, 0, (hid + 1) * sizeof(float));
//    memset(tmp_hidden_partial_sum, 0, num_blocks * WIDTH * sizeof(float));

//    for(int k = 0; k < hid + 1; k++){
// 	 tmp_output_hidden_cuda[k] = 0;
//    }
 
//    for(int k = 0; k < num_blocks * WIDTH; k++){
// 	tmp_hidden_partial_sum[k] = 0;
//    }

//    printf("dim: %d, num_blocks: %d\n", dim, num_blocks);
...
fd = open(h_str1, O_CREAT | O_RDWR | O_DIRECT, 0664);
if (fd < 0) {
	std::cerr << "file open error:" << std::endl;
}
memset((void *)&cf_descr, 0, sizeof(CUfileDescr_t));
...
ret = cuFileWrite(cf_handle, ret3.devPtr, ret3.alloc_size, 0, 0);	
if (ret < 0)
	printf("Error!!\n");
else {
	std::cout << "written bytes :" << ret << std::endl;
...
t_stop = high_resolution_clock::now();
duration = duration_cast<milliseconds>(t_stop - t_start);
std::cout << "Tid: " << tid << " Device to Storage copy time: " << duration.count() << std::endl;
