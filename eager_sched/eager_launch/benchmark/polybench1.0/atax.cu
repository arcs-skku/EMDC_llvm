...
void init_array(DATA_TYPE *x, DATA_TYPE *A, DATA_TYPE *y, DATA_TYPE *tmp)
{
	int i, j;
		...
		while(1){
			...
			if((chk_former_task == 1) && (full != 1)){
				...
				if(full == 1){
					...
					CUDA_CHECK(cudaMemAdvise(mem_list[1].devPtr, sizeof(DATA_TYPE) * NY, mem_list[1].advice, mem_list[1].device));
					CUDA_CHECK(cudaMemPrefetchAsync(mem_list[1].devPtr, sizeof(DATA_TYPE) * NY, 0, s_e));
					...
					CUDA_CHECK(cudaMemAdvise(mem_list[3].devPtr, sizeof(DATA_TYPE) * NX, mem_list[3].advice, mem_list[3].device));
					CUDA_CHECK(cudaMemPrefetchAsync(mem_list[3].devPtr, sizeof(DATA_TYPE) * NX, 0, s_e));
					break;
				}
			}
...
int main(int argc, char** argv)
{
	...
	atax_kernel1<<< grid1, block, 0, s1 >>>(A, x, tmp);
	...
	atax_kernel2<<< grid2, block, 0, s2 >>>(A, y, tmp);
	...
	CUDA_CHECK(cudaMemPrefetchAsync(y, sizeof(DATA_TYPE) * NY, cudaCpuDeviceId, 0));
	CUDA_CHECK(cudaDeviceSynchronize());
