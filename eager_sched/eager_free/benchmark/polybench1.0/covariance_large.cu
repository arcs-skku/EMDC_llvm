...
void covarianceCuda(int tid, DATA_TYPE* data, DATA_TYPE* symmat, DATA_TYPE* mean, DATA_TYPE* symmat_outputFromGpu)
{
	...
	// int64_t membytes = sizeof(DATA_TYPE) * ((M+1) * (N+1) + (M+1) * (M+1) + (M+1)) + 305 * 1024 * 1024;
	...
	bemps_begin(tid, find_max(grid1.x, grid2.x, grid3.x), find_max(grid1.y, grid2.y, grid3.y),
				find_max(grid1.z, grid2.z, grid3.z), find_max(block1.x, block2.x, block3.x), 
				find_max(block1.y, block2.y, block3.y), find_max(block1.z, block2.z, block3.z), membytes);
	...
	cudaDeviceSynchronize();
	cudaFree(mean_gpu);
	pre_bemps_free(tid, sizeof(DATA_TYPE) * (M+1));
...
int main(int argc, char* argv[])
{
	...
	covarianceCuda(tid, data, symmat, mean, symmat_outputFromGpu);
