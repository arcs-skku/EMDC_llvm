...
void correlationCuda(int tid, DATA_TYPE* data, DATA_TYPE* mean, DATA_TYPE* stddev, DATA_TYPE* symmat,
			DATA_TYPE* symmat_outputFromGpu)
{
	...
	int64_t membytes = 0;
	membytes += sizeof(DATA_TYPE) * ((M+1) * (N+1));
	membytes += sizeof(DATA_TYPE) * ((M+1) * (N+1));
	membytes += sizeof(DATA_TYPE) * ((M+1));
	membytes += sizeof(DATA_TYPE) * ((M+1));
	// int64_t membytes = sizeof(DATA_TYPE) * (int64_t)( (M+1) * (N+1) + (M+1) * (N+1) + (M+1) + (M+1) );
	...
	bemps_begin(tid, find_max(grid1.x, grid2.x, grid3.x, grid4.x), find_max(grid1.y, grid2.y, grid3.y, grid4.y), 
				find_max(grid1.z, grid2.z, grid3.z, grid4.z), find_max(block1.x, block2.x, block3.x, block4.x), find_max(block1.y, block2.y, block3.y, block4.y),
				find_max(block1.z, block2.z, block3.z, block4.z), membytes);
	...
	cudaFree(mean_gpu);
	cudaFree(stddev_gpu);
	pre_bemps_free(tid, sizeof(DATA_TYPE) * ((M+1) + (M+1)));
	...
	cudaDeviceSynchronize();
	cudaFree(data_gpu);
	pre_bemps_free(tid, sizeof(DATA_TYPE) * ((M+1) * (N+1)));
...
int main(int argc, char* argv[])
{
	...
	correlationCuda(tid, data, mean, stddev, symmat, symmat_outputFromGpu);
